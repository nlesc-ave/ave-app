...
export class RangeStepper extends React.Component<IProps, IState> {
  ...
  fetchSpecies() {
    fetch(`${this.props.apiroot}/species`)
      ...
      .catch((reason: TypeError) =>
        this.setState({ speciesError: reason.message })
      )
  }
  ...
  fetchGenomes(species: ISpecies) {
    fetch(`${this.props.apiroot}/species/${species.species_id}/genomes`)
      ...
      .then(allowedGenomes =>
        this.setState({ allowedGenomes, stepIndex: this.state.stepIndex + 1 })
      )
  }
  ...
  selectSpecies = (_event: React.FormEvent<{}>, selectedSpeciesId: string) => {
    const selectedSpecies = this.state.allowedSpecies.find(
      species => species.species_id === selectedSpeciesId
    )
  ...
  selectGenome = (_e: any, selectedGenomeId: string) => {
    const selectedGenome = this.state.allowedGenomes.find(
      genome => genome.genome_id === selectedGenomeId
    )
    if (!selectedGenome) {
      return
    }
  ...
  selectStart = (_e: any, selected: string) =>
    this.setState({ selectedStart: parseInt(selected, 10) })
  selectEnd = (_e: any, selected: string) =>
    this.setState({ selectedEnd: parseInt(selected, 10) })
  ...
  renderSpeciesStep = () => {
    ...
    const speciesRadios = this.state.allowedSpecies.map(species => (
      <RadioButton
    ...
    if (this.state.speciesError) {
      speciesError = (
        <span style={{ color: 'red' }}>{this.state.speciesError}</span>
      )
  ...
  renderGenomeStep = () => {
    ...
    const genomeRadios = this.state.allowedGenomes.map(genome => (
      <RadioButton
  ...
  renderRangeSearchStep = () => {
    ...
    const startError =
      this.state.selectedStart < 1 ? 'Start should be greater than 1' : ''
    let endError = ''
    ...
    if (
      ...
      this.state.selectedEnd > this.state.selectedChromosome.length
    ) {
      endError = `End should be smaller than chromosome length ${this.state
        .selectedChromosome.length}`
    }
    const chromosomeIds = selectedGenome.chromosomes.map(chr => chr.chrom_id)
