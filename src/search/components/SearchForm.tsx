...
export class SearchForm extends React.Component<IProps, IState> {
  ...
  constructor() {
    super()
    this.queryObservable = this.querySubject
  ...
  mapGeneAnnotation2Hit = (annotation: IGeneSearchResult) => {
    ...
    const route = `${this.props.genome_id}/${chrom}/${start - flank}/${end +
      flank}`
    return {
      key: annotation.id,
  ...
  fetchGeneAnnotations(query: string) {
    const url = `${this.props.apiroot}/genomes/${this.props
      .genome_id}/genes?query=${query}`
    return fetch(url)
  ...
  fetchFeatureAnnotations(query: string) {
    const url = `${this.props.apiroot}/genomes/${this.props
      .genome_id}/features?query=${query}`
    return fetch(url)
  ...
  render() {
    ...
    const hits = this.state.hits.map(
      ({ key, primaryText, secondaryText, route }) => (
        <ListItem
          key={key}
    ...
    return (
      <div>
        ...
        >
          <RadioButton value="genes" label="Gene" />
          <RadioButton value="features" label="Features" />
        </RadioButtonGroup>
        <TextField
          hintText={queryHint}
