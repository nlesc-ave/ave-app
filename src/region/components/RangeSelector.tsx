...
export class RangeSelector extends React.Component<IProps, {}> {
  ...
  onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      this.props.range.start !== this.state.start ||
      this.props.range.stop !== this.state.stop
    ) {
      const newRange = {
  ...
  render() {
    ...
    const chromosomeIds = chromosomes.map(c => c.chrom_id)
    const currentChromosome = chromosomes.filter(
      c => c.chrom_id === range.contig
    )[0]
    let error = ''
    if (start > stop) {
      ...
      <ToolbarGroup>
        ...
        <form onSubmit={this.onSubmit}>
          ...
          <TextField
            ...
            errorText={error}
          />
          <IconButton tooltip="GO" type="submit">
            <GoIcon />
          </IconButton>
        </form>
      </ToolbarGroup>
