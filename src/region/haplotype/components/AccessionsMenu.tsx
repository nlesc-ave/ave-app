...
export class AccessionsMenu extends React.Component<IProps, IState> {
    ...
    selectInvert = () => {
        ...
        const invertedSelection = this.props.accessions.filter((d) => !currentSelection.has(d));
    ...
    render() {
        const items = this.state.filtered.map(
            (d, i) => (
                <AccessionMenuItem
                    key={i}
                    accession={d}
    ...
    renderFilter = () => (
        <div>
            <TextField
                ...
                errorText={this.state.filtered.length === 0 ? 'No accessions matches filter' : ''}
            />
        </div>
    )
